...
char  PreviousFirstImageName[STRING_LENGTH*2];				// Name of first image to be processed in first video, stored in order to check if it is necessary to find a new Auto_Segmentation_Grey_Limit_From_First_Image
...
char  Write_0_MM_img, Write_1_SH_img, Write_2_SM_img, Write_3_SE_img, Write_4_CU_img, Write_5_ID_img, Write_6_MA_img, Write_7_VT_img;  // Flags set in command file to control output images written to file from various stages of the image processing. Normally enough with all except the last one. 
...
long  Nof__Cells[MAXLEVELS];								// Array with total number of identified cells in each image at each level
...
float Max_Cell_Shift_um;									// Longest allowed distance [µm] for a cell to move from one image to the next. Used to rule out fast moving contamination and to limit the search for matching cells. 
...
float     Auto_Segmentation_Grey_Limit_From_First_Image;	// The grey level in the background (when cells are masked) of the smoothed image which gives a 0.3 % count rate comparded to the most common grey level. Used for automatic segmentation. 
...
// Some arrays declared as global and used in cut_concave_region() and radius_of_curvature_for_fitted_circle()
short  Edge_Point_Matrix[MAXX][MAXY];
int    Edge_Point_X_px[CUT_MAX_EDGE_POINTS], Edge_Point_Y_px[CUT_MAX_EDGE_POINTS];		// Adding 2*CUT_MARGIN to size in order to be consistent in cut_concave_region()
double Edge_Point_Distances[CUT_MAX_EDGE_POINTS][CUT_MAX_EDGE_POINTS];	
...
int  fill_segmented_with_PMI(int fillnumber, int withnumber, int startx, int starty);  
...
void read_command_file_header(void)
{
	...
	if( (fgets(line, STRING_LENGTH, InFile)==NULL) || sscanf(line, "Valid track images contrast %f",    &VT_Image_Contrast)<1 || VT_Image_Contrast<=0 )						{ sprintf(message, "Missing or invalid image contrast factor %4.2f. Should be a positive number.", VT_Image_Contrast);													write_error_message(ErrorMessageFile, ERROR, message); }    else printf("Valid track images contrast factor: %3.1f (where > 1.0 gives contrast enhancement).\n", VT_Image_Contrast);	
	if( (fgets(line, STRING_LENGTH, InFile)==NULL) || sscanf(line, "Scalebar color %s",                 &string)<1 )														{ write_error_message(ErrorMessageFile, ERROR, "Missing command for 'Scalebar color'."); }																                else { strcpy(Scalebar_Color, string); printf("Will draw scale bar with %s color in images if scale bar image exists.\n", Scalebar_Color);  } 
	...
	if( (fgets(line, STRING_LENGTH, InFile)==NULL) || sscanf(line, "Segmentation limit [SDs] %f",       &Segmentation_Limit)<1  || Segmentation_Limit <0 )					{ sprintf(message, "Missing or invalid segmentation limit %4.2f. Should be a positive number.", Segmentation_Limit);													write_error_message(ErrorMessageFile, ERROR, message); }	else printf("Segmentation limit: %4.2f standard deviations from mean grey level.\n", Segmentation_Limit);	
...
//	if( (fgets(line, STRING_LENGTH, InFile)==NULL) || sscanf(line, "Longest image shift [um] %f",	    &Max_Image_Shift_um)<1 || Max_Image_Shift_um<0 )					{ sprintf(message, "Missing or invalid longest image shift %5.3f.", Max_Image_Shift_um);																				write_error_message(ErrorMessageFile, ERROR, message); }	else if( Max_Image_Shift_um == 0 ) printf("Will not align images since 'Longest image shift' = 0.\n"); else printf("Longest possible image shift: %3.1f um = %1.0f pixels.\n", Max_Image_Shift_um, Max_Image_Shift_um/Pixel_Size);
	...
	printf("Cutting_Diameter_px = %d \n", Cutting_Diameter_px); 
	Max_Cell_Shift_um = Max_Cell_Velocity * TimeStep * Image_Nr_Increment;		// Units: µm = µm/min * min * # timesteps to skip between images
	...
	fprintf(CMDWindowFile, "Image name,Shift X,Shift Y,Grey,Grey,SD based,Automatic,# cells,# cells\n");	// Writing header only once at the top two lines in the .csv file 
	...
	fprintf(CellCountFile,                                           ",,,,Image number,");		for( n=0; n<=Tuning__Last_Image; n++ ) fprintf(CellCountFile, "%d,", n                 );		fprintf(CellCountFile, "\n"); 
	fprintf(CellCountFile,                                           ",,,,Time [min],"  );		for( n=0; n<=Tuning__Last_Image; n++ ) fprintf(CellCountFile, "%3.1f,", n*TimeStep     );		fprintf(CellCountFile, "\n"); 
	fprintf(CellCountFile,                                           ",,,,Time [h],"    );		for( n=0; n<=Tuning__Last_Image; n++ ) fprintf(CellCountFile, "%5.3f,", n*TimeStep/60  ); 		fprintf(CellCountFile, "\n"); 
	fprintf(CellCountFile, "Video name,VID number,Well letter,Well number,Time [days]," );		for( n=0; n<=Tuning__Last_Image; n++ ) fprintf(CellCountFile, "%8.6f,", n*TimeStep/3600); 		fprintf(CellCountFile, "\n"); 
...
char read_image_series_info_from_bitmaps2process(void)
{
	...
	Max_Cell_Shift_um = Max_Cell_Velocity * TimeStep * Image_Nr_Increment;				// Updating value in case manual editing has set a different Image_Nr_Increment
...
{
	char imagename[STRING_LENGTH];
	int  i, n, x, y, top_margin, right_margin; 
	...
	if( read_bitmap(Rimage, imagename) ) {					// ...or no more images in the series are found 
		...
		if( level%30 == 0 ){								// Writing the header to screen every 20 levels so the info is always visible 
			printf("\n");   
			printf("                                  Shift X   Shift Y   Grey    Grey     SD based      Automatic    # cells    # cells \n");   
			printf("      Image name                   [um]      [um]     mean     SD     grey limit     grey limit   in image   matched \n");   
		}

		// Making a working copy in Bimage for cropping etc. 
		copy_bitmap(Rimage, Bimage); 

/*		// Simple smooting filter to make images with high contrast regions look more similar if some are shifted much and thus pixels are mixed more in the fractional shifting above. THIS DID NOT REDUCE THE CELL CM MOVEMENTS FOR FIXED CELLS 2022-01-24 SO DROPPED SINCE IMAGES GOT CLEARLY MORE BLURRED
		...
		// Copying image (which now has a new size in x and y) to shifted position
...
//			Oimage->xsize = (long)2*(Bimage->xsize + SHIFTXY); if( Oimage->xsize > MAXX ) Oimage->xsize = MAXX;	
//			Oimage->ysize = (long)2*(Bimage->ysize + SHIFTXY); if( Oimage->ysize > MAXY ) Oimage->ysize = MAXY;	
			Oimage->xsize = (long)(Bimage->xsize + 2*SHIFTXY); if( Oimage->xsize > MAXX ) Oimage->xsize = MAXX;		// New 2023-06-01 to get smaller images and save disk space
...
void smooth_images_and_identify_cells(int level) 
{
	...
	if( level==0 && Tuning_Image_Code<2 ) { 
		...
		if( FALSE && !strcmp(current_first_image_name, PreviousFirstImageName) ) { 		// TEMPORARILY NOT DOING THIS TEST SO ALL RUNS WILL FIND OPTIMAL GREYLEVEL IN CASE THE Segmentation_Limit HAS BEEN CHANGED MANUALLY IN THE SETTINGS FILE
			Auto_Segmentation_Grey_Limit_From_First_Image = Previous_Segmentation_Grey_Limit_From_First_Image; 
		} else {
	...
	//		cc = B*B - 4.0*A*(C+2.5);	// +2.5 corresponds to a count at this greylevel which is 0.3 % of the max value in the nearly gaussian grey level distribution in the filtered background (+1 would be 10 %, +2 would be 1 %, +3 would be 0.1 %, etc.) 
			...
			if( cc>0 && A<0 ) { 
				dd = 0.5*(-B + sqrt(cc))/A;		
				Auto_Segmentation_Grey_Limit_From_First_Image = (float)(grey_mode+(center_i+dd-histogram_half_bins)/histogram_zoom_factor);					
			} else { 
				...
				Auto_Segmentation_Grey_Limit_From_First_Image = (float)user_segmentation_grey_limit;
			} 
			...
			fprintf(TuneFile, "First image grey level limit %5.3f\n", Auto_Segmentation_Grey_Limit_From_First_Image);
			fprintf(TuneFile, "First image path and name    %s\n",    PreviousFirstImageName);
			fclose(TuneFile);	//*/
		}
	} // End of SECOND "	if( level==0 && Tuning_Image_Code<2 )"

	if( Tuning_Image_Code > 1 ) { 		// In order to use the Auto_Segmentation_Grey_Limit_From_First_Image also when analyzing middle and last tuning image, reading the segmentation limit from disk instead of doing the above analyses. Since tuning always starts with the first selected image, the correct segmentation limit exists in the file. 
		sprintf(filename, "C:/celltraxx_system/celltraxx_tuning_info.txt"); 
		if( (TuneFile=fopen(filename, "r"))==NULL ){ sprintf(message, "Can't open file celltraxx_tuning_info.txt for reading.\nPlease check if the file is open in another application.\n");  write_error_message(ErrorMessageFile, ERROR, message); }
		if( (fgets(line, STRING_LENGTH, TuneFile)==NULL) || sscanf(line, "First image grey level limit %f", &Auto_Segmentation_Grey_Limit_From_First_Image)<1  || Auto_Segmentation_Grey_Limit_From_First_Image <0 ) { sprintf(message, "Missing or invalid value for 'First image grey level limit' (%5.3f) in file %s. Should be a positive number.", Auto_Segmentation_Grey_Limit_From_First_Image, filename); write_error_message(ErrorMessageFile, ERROR, message); }
		fclose(TuneFile);
	}

	if( Auto_Segmentation_Grey_Limit_From_First_Image > smean ) {			// If auto segmentation clearly fails, using instead the SD based value of greylevel_mean - Segmentation_Limit*greylevel_SD 
		printf("Warning: The automatic segmentation limit (%4.2f) is brighter than the mean grey level (%4.2lf).\nInstead using the SD based segmentation limit (%4.2lf)\n", Auto_Segmentation_Grey_Limit_From_First_Image, smean, user_segmentation_grey_limit); 
		Auto_Segmentation_Grey_Limit_From_First_Image = (float)user_segmentation_grey_limit;
	} 
	printf("%6.2f  %5.2f      %6.2f         %6.2f       ", smean, ssd, user_segmentation_grey_limit, Auto_Segmentation_Grey_Limit_From_First_Image); 
	fprintf(CMDWindowFile, "%5.3f,%5.3f,%5.3f,%5.3f,",     smean, ssd, user_segmentation_grey_limit, Auto_Segmentation_Grey_Limit_From_First_Image); 
	...
	// Segmenting pixels in the Smoothed_Image[][] that are darker than Auto_Segmentation_Grey_Limit_From_First_Image 
	for( y=0; y<Bimage->ysize; y++ )
		for( x=0; x<Bimage->xsize; x++ ) 
			if(   Smoothed_Image[SHIFTXY+x][SHIFTXY+y] < Auto_Segmentation_Grey_Limit_From_First_Image ) Segmented_Image[SHIFTXY+x][SHIFTXY+y] = 1;		// NB: Coding segmented pixel as 1 to distinquish from NEGATIVE numbered cells below 
			else Segmented_Image[SHIFTXY+x][SHIFTXY+y] = 255;														// Temporarily coding background as 255 to distinquish holes in cells from the continuous background after filling it with 0 below 
		...
		for( y=0; y<Bimage->ysize; y++ )
			for( x=0; x<Bimage->xsize; x++ ) 
				if( Segmented_Image[SHIFTXY+x][SHIFTXY+y] == CUT ) {
					Oimage->image[Left___Crop_Margin+x][Bottom_Crop_Margin+y][0] = Oimage->image[Left___Crop_Margin+x][Bottom_Crop_Margin+y][1] = (unsigned char)0; Oimage->image[Left___Crop_Margin+x][Bottom_Crop_Margin+y][2] = (unsigned char)255;  						// Red cutting lines		
				} else if( Segmented_Image[SHIFTXY+x][SHIFTXY+y] == CUT2 ) {
					Oimage->image[Left___Crop_Margin+x][Bottom_Crop_Margin+y][0] = Oimage->image[Left___Crop_Margin+x][Bottom_Crop_Margin+y][1] = (unsigned char)0; Oimage->image[Left___Crop_Margin+x][Bottom_Crop_Margin+y][2] = (unsigned char)0;  							// Black all potential cut lines		
				} else if( Segmented_Image[SHIFTXY+x][SHIFTXY+y] == FIT ) {
					Oimage->image[Left___Crop_Margin+x][Bottom_Crop_Margin+y][0] = (unsigned char)0; Oimage->image[Left___Crop_Margin+x][Bottom_Crop_Margin+y][1] = Oimage->image[Left___Crop_Margin+x][Bottom_Crop_Margin+y][2] = (unsigned char)255;  						// Yellow		
				} else if( Segmented_Image[SHIFTXY+x][SHIFTXY+y] == FIT2 ) {
					Oimage->image[Left___Crop_Margin+x][Bottom_Crop_Margin+y][0] = (unsigned char)0; Oimage->image[Left___Crop_Margin+x][Bottom_Crop_Margin+y][1] = (unsigned char)127; Oimage->image[Left___Crop_Margin+x][Bottom_Crop_Margin+y][2] = (unsigned char)255;  	// Orange 		
				} else if( Segmented_Image[SHIFTXY+x][SHIFTXY+y] == FIT3 ) {
					Oimage->image[Left___Crop_Margin+x][Bottom_Crop_Margin+y][0] = (unsigned char)255; Oimage->image[Left___Crop_Margin+x][Bottom_Crop_Margin+y][1] = (unsigned char)0; Oimage->image[Left___Crop_Margin+x][Bottom_Crop_Margin+y][2] = (unsigned char)255;  	// Magenta		
					...
					Oimage->image[Left___Crop_Margin+x][Bottom_Crop_Margin+y][0] = Oimage->image[Left___Crop_Margin+x][Bottom_Crop_Margin+y][2] = (unsigned char)0; Oimage->image[Left___Crop_Margin+x][Bottom_Crop_Margin+y][1] = (unsigned char)255;  						// Green cell outlines		
				}
		if( Add_Scalebar ) draw_scalebar(); 
...
//	printf("Running cut_concave_region() with x=%5d y=%5d at level=%4d with Nof__Cells = %4d   xend = %4d   yend = %4d \n", start_x, start_y, level, ss, xend, yend); 
	...
	do {
		...
		if( !found ) return(FALSE); // { printf("Error in cut_conca ve_region(): Could not find the next edge point even if the search spiral \nreached an edge length of %d pixelsfor the region beginning at (%4d,%4d).\nPlease increases MAX_SEARCH_SPIRAL_EDGE and recompile.\n", e, start_x+xmin, start_y+ymin); exit(TRUE); }
			...
			if( ++nof_edge_points == max_nof_edge_points ) { printf("Aborting cut_concave_region() since maxumum allowed number of edge points (%d) was reached for region starting at pixel [%d,%d]\n", max_nof_edge_points, start_x+xmin, start_y+ymin); return(FALSE); }  
...
/*			// Temporarily drawing bounding boxes around all matched cells  
		...
		Nof_matched_Cells = Nof__Cells[0]; 
		printf(         "%5d     %5d \n", Nof__Cells[0], 0);
...
//		printf("\nNof__Cells[level-1] = %4d    Nof__Cells[level] = %4d \n", Nof__Cells[level-1], Nof__Cells[level]); 
		...
		// Repeatedly finding the lowest distance value in the  Matching_Matrix[][] and assuming it represents the same cell in this level and the previous. Continuing until the last matched cell movement is above the limit Max_Cell_Shift_um
		cells_matched = TRUE; 
		do{
...
/*				// Temporarily drawing bounding boxes around all matched cells  
			...
			// Looking for ordinary matching of one cell with itself in the next layer: Finding and matching the closest candidate until the given Max_Cell_Shift_um limitation is reached 
			} else if( smallest_shift_1*Pixel_Size < Max_Cell_Shift_um ){	 

//				printf("\nlevel = %3d   i_1 = %4d   j_1 = %4d   smallest_shift_1 = %8.3f px = %8.3f um   Max_Cell_Shift_um = %8.3f um   Nof__Cells[level-1] = %d    Nof__Cells[level] = %d \n", level, i_1, j_1, smallest_shift_1, smallest_shift_1*Pixel_Size, Max_Cell_Shift_um, Nof__Cells[level-1], Nof__Cells[level]); 
		...
		for( j_1=1; j_1<=Nof__Cells[level]; j_1++ ){
			...
			if( Matching_Matrix[MAXCELLS][j_1] < BIG-1 ){			// Info on which cells in this level that have been matches is stored in the last column of the Matching_Matrix[][]
				...
				if( level > 1 ) { 
					...
					if( smallest_shift_1 < 2*Max_Cell_Shift_um && Matched_Cells[level-1][ Cells[level-2][i_1].cellnumber ]==0 ){	// Found a cell two levels below with separation less twice the maximum step between two consecutive images and which is not matched to a cell one level below. 
...
//		Velocity_Matrix[level+8][1] =  Velocity_Matrix[level+7][1]/sqrt((double)m);						// SEM of Euclidean velocity  
	...
	for( n=First_Image_Number; n<=First_Image_Number+level-1; n++ ) fprintf(CellCountFile, "%d,", Nof__Cells[n-First_Image_Number]);		// In the rare cases where First_Image_Number+level-1 > Tuning__Last_Image, there will not be time values above the last columns of counts for this row. But better to give all cell counts than to trunkate.
