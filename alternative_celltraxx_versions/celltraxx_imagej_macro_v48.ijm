...
// (C) Børge Holme, SINTEF, 2024-09-22 - CellTraxx version 4.8 
...
var perform_cell_shape_analysis;
...
entries=split(textlines[i++]," \t");		if( entries[4]=="yes" || entries[4]=="Yes" || entries[4]=="YES" ) perform_cell_shape_analysis    = true; else perform_cell_shape_analysis    = false; 
...
if( perform_interactive_tuning ) {
	...
	exec("cmd /c start "+sys_dir+"celltraxx_v48_2024-09-22_VS2022_x64_Cell-shape");    					// "start" will open command window for output during execution 
...
/*		// Option to draw lines around button areas to check location on image 
		...
		if( button_number == 10 ) {			// [Run]				
			...
			runMacro(sys_dir+"celltraxx_imagej_macro_v48.ijm"); 	// For simplicity, just running macro recursively when [Back] button is pressed 
			exit; 
		} 
		...
		if( button_number == 12 ) { 	// [Cancel]
			...
			exec("cmd /c start "+sys_dir+"celltraxx_v48_2024-09-22_VS2022_x64_Cell-shape");    													// "start" will open DOS window for output during execution 
			filestring = File.openAsString(sys_dir+"celltraxx_error_messages.txt");							// Checking if celltraxx.exe left a warning or error message, if so exiting and writing the error text to a message box
			if(      lengthOf(filestring) > 11 ) { close("CellTraxx Tuning Image"); exit(filestring); }  	// Exiting macro after closing tuning window and writing the error message from celltraxx.exe to message box
			else if( lengthOf(filestring) == 0 ) { close("CellTraxx Tuning Image"); exit("An unidentified error caused celltraxx.exe to stop running."); }
		}
...
print("\nRunning the CellTraxx program. This can take some time. Check status in the command window C:\\celltraxx_system\\celltraxx_v48_2024-09-22_VS2022_x64_Cell-shape.exe");
exec("cmd /c start "+sys_dir+"/celltraxx_v48_2024-09-22_VS2022_x64_Cell-shape");   														// "start" will open command window for output during execution 
filestring = File.openAsString(sys_dir+"celltraxx_error_messages.txt");								// Checking if celltraxx.exe left a warning or error message, if so exiting and writing the error text to a message box
	if(      lengthOf(filestring) > 11 ) { close("CellTraxx Tuning Image"); exit(filestring); }  	// Exiting macro after closing tuning window and writing the error message from celltraxx.exe to message box
	else if( lengthOf(filestring) == 0 ) { close("CellTraxx Tuning Image"); exit("An unidentified error caused celltraxx.exe to stop running."); }
...
function Write_Settings_To_File()
{
	...
	if( perform_cell_shape_analysis	    ) string = "yes"; else string = "no"; File.append(    "Perform cell shape analysis     "+string,      sys_dir+"celltraxx_defaults.txt");		
